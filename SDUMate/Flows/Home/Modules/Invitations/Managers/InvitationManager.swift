...
final class InvitationManager {
    ...
    func createSession(invitation: Invitation) -> Promise<Void> {
        return Promise { seal in
            ...
            let session = Session(id: documentRef.documentID, announcerId: invitation.announcerId, respondentId: invitation.respondentId, announcementId: invitation.announcementId, announceType: invitation.announcement?.type ?? .request, status: .active, createdDate: Date().toString())
